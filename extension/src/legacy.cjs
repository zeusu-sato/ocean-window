...
function createController(vscode, context, dependencies = {}) {
  ...
  async function restoreCurrent({ preserveSceneState = false } = {}) {
    requireDesktop();
    const report = await withReceiptLock(async () => {
      const { runInstall } = await loadInstaller();
      ...
      const plan = await runInstall({ ...options, dryRun: true });
      log(plan);
      // Failed first enables can leave a receipt on an otherwise untouched,
      // read-only installation. A clean app must never need a native lock.
      if (!plan.changed) {
        if ((await receipts()).installs.some(entry => entry.appRoot === appRoot)) await saveState('disabled');
        return { ...plan, dryRun: false, reloadRequired: false };
      }
    ...
    if (!preserveSceneState) await context.globalState.update('oceanWindow.enabled', false);
    return report;
  }
  async function disable() {
    const report = await restoreCurrent();
    offerReload(report);
    return report;
  }
  ...
  return { register, enable: command(enable), disable: command(disable), status: command(status),
    inspectLegacy: () => serialize(inspect),
    restoreLegacy: () => serialize(() => restoreCurrent({ preserveSceneState: true })),
